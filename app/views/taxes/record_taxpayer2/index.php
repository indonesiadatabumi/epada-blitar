...
<section class="portfolio-showcase-block min-height-maincontent">
	<div class="box">
		<div class="row">
			<div class="col-md-6">
				<h5>
					<i class="fa fa-database"></i> Rekam WP Badan Usaha
				</h5>
			</div>
			<div class="col-md-6">
				<ul class="breadcrumb">
						...
						<div class="form-actions">
							<div class="row">
								<div class="col-md-10 col-md-offset-2">
									...
									<button class="btn btn-default" type="button" id="btn-add1" data-toggle="modal" data-target="#nonnpwpdModal" onclick="load_nonnpwpd_content(this.id)"><i class="fa fa-list-alt"></i> List WP non NPWPD</button>
									<?php
									if ($add_priv == '0') {
			...
			<div class="modal-dialog modal-lg" style="width:1200px!important;">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title" id="defModalHead">Form WP Badan Usaha</h4>
					</div>
		...
		<!-- MODAL -->
		<div class="modal fade" id="nonnpwpdModal" role="dialog">
			<div class="modal-dialog modal-lg" style="width:1200px!important;">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
						<h4 class="modal-title" id="defModalHead">List Wp Non Npwpd</h4>
					</div>
					<div class="modal-body">
						<div class="row">
							<div id="loader-nonnpwpdModal" class="" align="center"><img src="<?= $this->config->item('img_path'); ?>ajax-loaders/ajax-loader-1.gif" /></div>
							<div class="col-md-12 col-sm-12 col-xs-12" id="content-nonnpwpdModal">
							</div>
						</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	function get_zona(district_val, content_id) {
		ajax_object.reset_object();
		var data_ajax = new Array('district=' + district_val);
		ajax_object.set_url(base_url + 'glob/get_zona').set_data_ajax(data_ajax).set_content('#' + content_id).request_ajax();
	}

	function get_sda(zona_rows, content_id) {
		ajax_object.reset_object();
		var data_ajax = new Array('zona_rows=' + zona_rows);
		ajax_object.set_url(base_url + 'glob/get_sda').set_data_ajax(data_ajax).set_content('#' + content_id).request_ajax();
	}
	...
	function load_nonnpwpd_content() {
		pajak_id = $('#bundle_id').val();
		data_ajax = ['pajak_id=' + pajak_id];
		ajax_object.set_url(base_url + 'bundle/taxes/' + bundle_item_type + '/' + menu + '/nonnpwpd')
			.set_data_ajax(data_ajax)
			.set_loading('#loader-nonnpwpdModal')
			.set_content('#content-nonnpwpdModal')
			.request_ajax();
	...
	function balik_nama(id) {
		...
		ajax_object.set_url(base_url + 'bundle/taxes/' + bundle_item_type + '/' + menu + '/balik_nama')
			.set_plugin_datatable(true)
			.set_id_input(id)
			.set_input_ajax('ajax-req-dt')
			.set_data_ajax(data_ajax)
			.set_loading('#preloadAnimation').set_content('#list-data').update_ajax('balik nama');
	}
